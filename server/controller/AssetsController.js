...
router.get("/", (req, res, next) => {
  Asset.find()
    .sort({ modified_dt: -1, assetNumber: -1 })
    .exec()
    .then((docs) => {
      // console.log(docs);
      if (docs.length > 0) {
      ...
      if (existingAsset) {
        ...
        try {
          ...
          if (!existingSerial){
            ...
            stats.push("Asset with serial number: "+ data[i].serial_number + " has been added.")
          } else {
            const updateOps = {};
            for (const [key, value] of Object.entries(asset)) {
              updateOps[key] = value;
            ...
            Asset.updateOne(
              ...
              .exec()
              .then(
                stats.push("Asset with serial number: "+ updateOps["_doc"].serial_number + " has been updated.")
              )
              .catch((err) => {
                stats.push(err)
              });
        ...
        asset.save(asset)
        stats.push(asset.assetNumber + " has been added.")
      }
    } catch (err) {
      stats.push(err)
    }
